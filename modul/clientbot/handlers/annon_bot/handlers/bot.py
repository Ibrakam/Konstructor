...
from modul.clientbot.handlers.annon_bot.states import Links
from modul.clientbot.handlers.annon_bot.keyboards.buttons import *
from modul.clientbot.handlers.annon_bot.shortcuts.userservice import get_channels_for_check, get_greeting, check_user, \
    add_user, get_user_by_link, add_link_statistic, get_user_link, change_greeting_user, add_messages_info, check_link, \
    change_link_db, check_reply, add_answer_statistic, get_all_statistic
...
async def start_anon_bot(message: Message, state: FSMContext, command: BotCommand = None):
    ...
    if not channels_checker:
        if not checker:
            ...
            await add_user(message.from_user.id, link_for_db)
    else:
        if not checker:
            new_link = await create_start_link(message.bot, str(message.from_user.id), encode=True)
            link_for_db = new_link[new_link.index("=") + 1:]
...
async def call_backs(query: CallbackQuery, state: FSMContext):
    ...
    if query.data == "cancel":
        ...
        await query.bot.delete_message(chat_id=query.from_user.id, message_id=query.message.message_id)
        await change_greeting_user(tg_id=query.from_user.id)
    ...
    if message.text:
        ...
        if 4 < len(new_greeting) < 301:
            ...
            await message.bot.send_message(chat_id=message.from_user.id, text=f"Отлично!\n\n"
                                           ...
                                           reply_markup=await main_menu_bt())
            await change_greeting_user(message.from_user.id, new_greeting)
            await state.clear()
    else:
        await message.bot.send_message(chat_id=message.from_user.id,
...
async def any_or_answer(message: Message, state: FSMContext, bot: Bot):
    ...
    if shortcuts.have_one_module(bot_db, "anon"):
        channels_checker = await check_channels(message)
        checker = await check_user(message.from_user.id)
        check = None
            ...
            if message.text == "☕️Поддержать разработчика":
                ...
                await message.bot.send_message(
                    ...
                    text=(
                        ...
                        f'⭐ Популярность сегодня: {statistic.get("position_today")} место\n'
                        ...
                        f'⭐ Популярность за всё время: {statistic.get("position_overall")} место\n\n'
                        f'Для повышения ⭐ популярности необходимо увеличить количество переходов по вашей ссылке.\n\n'
                        f'@{bot_username}'
                    ),
                    parse_mode="html",
                    reply_markup=await main_menu_bt()
