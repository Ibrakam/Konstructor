from aiogram import Router, F, Bot as Bottg
from aiogram.filters import Command
from aiogram.fsm.context import FSMContext
from aiogram.types import Message, CallbackQuery
from modul.clientbot.handlers.annon_bot.keyboards.buttons import *
from modul.clientbot.handlers.annon_bot.states import ChangeAdminInfo
from modul.clientbot.handlers.annon_bot.shortcuts.adminservice import *
from modul.loader import client_bot_router


@client_bot_router.message(Command(commands=["admin"]))
async def admin_mm(message: Message, bot: Bottg):
    bot_token = await bot.get_me()
    admin_id = await get_admin_id(bot_token.token)
...
async def delete_channel(message: Message, state: FSMContext):
    if message.text == "❌Отменить":
        ...
        try_del = await delete_channel_db(int(message.text))
        if try_del:
